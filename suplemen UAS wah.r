...
#another
f <- function(x){x} #Function to be integrated over [a,b].

Integral <- function(n,a,b,f){    #Large n gives better approximation.
  X <- runif(n,a,b)               #Use Unif[a,b] as reference pdf.
  Y <- (f(X))/(1/(b-a))
  Int <- sum(Y)/n                 #Int. Approx.
  X <- runif(n,a,b)               #SE of Int and 95% CI
  YY <- (f(X)/(1/(b-a)))^2
  SE <- sqrt((sum(YY)/n-Int^2)/n)
  CI <- c(Int-1.96*SE,Int+1.96*SE)
  list("Int"=Int,"SE"=SE, "95% CI"=CI)
}

#discontinues
f <- function(x){ifelse( (x < 1 ),1,2)} #Discontinuous function.
x <- seq(0,2,0.01)
